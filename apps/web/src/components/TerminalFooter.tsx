...
export function TerminalFooter() {
  ...
  const statusLabels: Record<string, string> = {
    LIVE: dict.common.status.live,
    STALE: dict.common.status.stale,
    RECONNECTING: dict.common.status.reconnecting,
    CONNECTING: dict.common.status.connecting,
  };
  const statusLabel = statusLabels[status] ?? dict.common.status.offline;
      ...
      <div className="flex items-center gap-3">
        <span className="flex items-center gap-1.5">
          <span
            className={
              status === "LIVE"
                ? "w-1.5 h-1.5 rounded-full bg-emerald-400 inline-block"
