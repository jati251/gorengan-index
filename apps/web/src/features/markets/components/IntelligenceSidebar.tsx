...
import { Activity, Newspaper, Layers } from "lucide-react";
...
export type IntelligenceTab = "orderbook" | "pulse" | "news";
...
export function IntelligenceSidebar({ className, initialTab = "orderbook" }: IntelligenceSidebarProps) {
  ...
  return (
    <div className={clsx("flex flex-col h-full overflow-hidden select-none font-mono", className)}>
      {/* Tab Switcher Header */}
      <div className="p-2 border-b border-white/[0.08] bg-[#3c3f5f] shrink-0">
        <div className="flex items-center gap-1 p-1 bg-white/[0.03] border border-white/[0.07] rounded-xl relative">
          <button
            type="button"
            onClick={() => setActiveTab("orderbook")}
            className={clsx(
              "relative flex-1 flex items-center justify-center gap-1.5 py-1.5 px-2 rounded-lg text-xs font-mono font-medium transition-all cursor-pointer z-10 border",
              activeTab === "orderbook"
                ? "bg-emerald-500/15 border-emerald-500/30 text-emerald-300 font-semibold shadow-xs"
                : "border-transparent text-slate-400 hover:text-slate-200 hover:bg-white/[0.04]"
            )}
          >
            <Layers className="w-3.5 h-3.5 text-emerald-400 shrink-0" />
            <span className="truncate">{dict.intelligence.tabs.orderbook}</span>
          </button>
          ...
          >
            ...
            <span className="truncate">{dict.intelligence.tabs.stats}</span>
          </button>
          ...
          >
            ...
            <span className="truncate">{dict.intelligence.tabs.news}</span>
          </button>
        </div>
      </div>
